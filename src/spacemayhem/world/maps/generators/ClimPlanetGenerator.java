package spacemayhem.world.maps.generators;
...
public class ClimPlanetGenerator extends PlanetGenerator{
    ...
    Block[][] arr =
            {
                    {SpaceMayhemBlocks.gasFloor}
            };
    {
        baseSeed = 3;
        defaultLoadout = SMLoadouts.basicIsland;
    }

    ObjectMap<Block, Block> dec = ObjectMap.of(
            Blocks.tar, Blocks.stone,
            Blocks.grass
    );
            ...
            for(Ptile other : tile.tiles){
                ...
                if(
                        osec.id == sector.planet.startSector || //near starting sector
                                osec.generateEnemyBase && poles < 0.8 || //near other base
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == Blocks.sand || floor == Blocks.salt ? SpaceMayhemBlocks.gasFloor : Blocks.tar));
                                }
                            }
                        }
                    }
        ...
        for(int i = 0; i < tlen; i++){
            ...
            if(tile.block() == Blocks.air){
                total ++;
                if(tile.floor().liquidDrop == Liquids.oil){
                    waters ++;
                }
            }
        }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                boolean spore = floor != Blocks.darksand && floor != Blocks.salt;
                ...
                if(!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow || floor.asFloor().isLiquid)){
                    floor = spore ?
                            (deep ? SpaceMayhemBlocks.gasFloor : SpaceMayhemBlocks.gasFloor) :
                            (deep ? SpaceMayhemBlocks.gasFloor :
                                    (floor == Blocks.darksand || floor == Blocks.salt ? SpaceMayhemBlocks.gasFloor : Blocks.tar));
                }
            }
        });
        ...
        pass((x, y) -> {
            ...
            if(floor == Blocks.hotrock){
                ...
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? Blocks.tar : (floor == Blocks.sand ? SpaceMayhemBlocks.gasFloor : Blocks.stone);
                    }else{
            ...
            Block[] liquidsTiles = new Block[]{SpaceMayhemBlocks.gasFloor};
            for (Block b : liquidsTiles) {
                if (floor == b) {
                    if (Math.abs(0.5f - noise(x - 40, y, 2, 0.7, 80)) > 0.25f &&
                        ...
                        floor = SpaceMayhemBlocks.gasScrapFloor;
                    }
                }
            }
        ...
        state.rules.enemyCoreBuildRadius = 700f;
